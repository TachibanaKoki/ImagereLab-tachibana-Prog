...
namespace Framework
{
		...
		HINSTANCE hInstance)
	{
		...
		m_pCtx->QueryInterface(&m_pAnnotation);
		...
		CHECK_D3D(m_pDevice->CreateDepthStencilState(&dssDesc, &m_pDssDepthTestReverse));
		...
		D3D11_BLEND_DESC bsMultiplicativeDesc =
		{
			false, false,
			{
				true,
		...
		CHECK_D3D(m_pDevice->CreateBlendState(&bsMultiplicativeDesc, &m_pBsMultiplicative));
		...
		D3D11_BLEND_DESC bsAlphaToCoverageDesc =
		{
			true, false,
			{
				false,
		...
		CHECK_D3D(m_pDevice->CreateBlendState(&bsAlphaToCoverageDesc, &m_pBsAlphaToCoverage));
	...
	void D3D11Window::SetViewport(ID3D11DeviceContext * pCtx, int2_arg dims)
	{
		...
		D3D11_RECT rect = { 0, 0, dims.x, dims.y };
		pCtx->RSSetScissorRects(1, &rect);
	}
	...
	void D3D11Window::SetViewport(ID3D11DeviceContext * pCtx, float2 origin, float2 size)
	{
		D3D11_VIEWPORT vp =
		{
			origin.x, origin.y,
	...
	void D3D11Window::BeginPerfEvent(LPCWSTR name)
	{
		if (m_pAnnotation)
			m_pAnnotation->BeginEvent(name);
	}

	void D3D11Window::EndPerfEvent()
	{
		if (m_pAnnotation)
			m_pAnnotation->EndEvent();
	}

	D3D11Window::PerfSection::PerfSection(D3D11Window * pWindow, LPCWSTR name)
		: m_pWindow(pWindow)
	{
		m_pWindow->BeginPerfEvent(name);
	}

	D3D11Window::PerfSection::~PerfSection()
	{
		m_pWindow->EndPerfEvent();
	}
}
