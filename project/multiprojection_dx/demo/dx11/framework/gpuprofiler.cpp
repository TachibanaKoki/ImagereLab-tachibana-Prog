...
namespace Framework
{
	...
	void GPUProfiler::OnFrameEnd(ID3D11DeviceContext * pCtx)
	{
		...
		{
			m_msSum[i] += 1000.0f * double(timestamps[i+1] - timestamps[i]) / double(disjointData.Frequency);
		}

		// The last element is the whole-frame time
		m_msSum[m_markerCount] += 1000.0f * double(timestamps[m_markerCount+1] - timestamps[0]) / double(disjointData.Frequency);
