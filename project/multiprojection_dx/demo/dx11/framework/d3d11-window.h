...
namespace Framework
{
	...
	public:
		...
		void				SetViewport(ID3D11DeviceContext * pCtx, float2 origin, float2 size);
		...
		// Perf annotations interface
		void				BeginPerfEvent(LPCWSTR name);
		void				EndPerfEvent();

		struct PerfSection
		{
			D3D11Window* m_pWindow;
			PerfSection(D3D11Window* pWindow, LPCWSTR name);
			~PerfSection();
		};

		#define PERF_SECTION(NAME) PerfSection PerfSection_##NAME(this, L#NAME)
		...
		comptr<ID3D11DepthStencilState>		m_pDssDepthTestReverse;
		...
		comptr<ID3D11BlendState>			m_pBsMultiplicative;
		comptr<ID3D11BlendState>			m_pBsAlphaBlend;
		comptr<ID3D11BlendState>			m_pBsAlphaToCoverage;
