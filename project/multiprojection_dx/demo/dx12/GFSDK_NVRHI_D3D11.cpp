...
namespace NVRHI
{
    ...
    {
        context->UpdateSubresource(t->resource.Get(), subresource, NULL, data, rowPitch, depthPitch);
    }
	...
	void RendererInterfaceD3D11::resolveTexture(TextureHandle dst, TextureHandle src, Format::Enum format, uint32_t dstSubres, uint32_t srcSubres)
	{
    ...
    void RendererInterfaceD3D11::writeBuffer(BufferHandle buffer, const void* data, size_t dataSize)
    {
    ...
    void RendererInterfaceD3D11::copyToBuffer(BufferHandle dest, uint32_t destOffsetBytes, BufferHandle src, uint32_t srcOffsetBytes, size_t dataSizeBytes)
    {
        ...
        srcBox.front = 0;
        srcBox.back = 1;
        context->CopySubresourceRegion(dest->resource.Get(), 0, (UINT)destOffsetBytes, 0, 0, src->resource.Get(), 0, &srcBox);
    }

    void RendererInterfaceD3D11::readBuffer(BufferHandle buffer, void* data, size_t* dataSize)
    {
        if (!dataSize)
            return;
...
#define D3D11_FILTER_REDUCTION_TYPE_STANDARD 0
    ...
    void RendererInterfaceD3D11::drawIndirect(const DrawCallState& state, BufferHandle indirectParams, uint32_t offsetBytes)
    {
        ...
        context->DrawInstancedIndirect(indirectParams->resource.Get(), offsetBytes);
    ...
    void RendererInterfaceD3D11::dispatchIndirect(const DispatchState& state, BufferHandle indirectParams, uint32_t offsetBytes)
    {
        ...
        context->DispatchIndirect(indirectParams->resource.Get(), (UINT)offsetBytes);
    ...
    void RendererInterfaceD3D11::getClearViewForTexture(TextureHandle texture, uint32_t index, bool asUINT, ID3D11UnorderedAccessView*& outUAV, ID3D11RenderTargetView*& outRTV, ID3D11DepthStencilView*& outDSV)
    {
        outUAV = NULL;
        outRTV = NULL;
        outDSV = NULL;
    ...
    ID3D11ShaderResourceView* RendererInterfaceD3D11::getSRVForTexture(TextureHandle texture, DXGI_FORMAT format, uint32_t mipLevel)
    {
    ...
    ID3D11RenderTargetView* RendererInterfaceD3D11::getRTVForTexture(TextureHandle texture, uint32_t arrayItem, uint32_t mipLevel)
    {
    ...
    ID3D11DepthStencilView* RendererInterfaceD3D11::getDSVForTexture(TextureHandle texture, uint32_t arrayItem, uint32_t mipLevel)
    {
    ...
    ID3D11UnorderedAccessView* RendererInterfaceD3D11::getUAVForTexture(TextureHandle texture, DXGI_FORMAT format, uint32_t mipLevel /*= 0*/)
    {
		if (format == DXGI_FORMAT_UNKNOWN)
		{
			format = GetFormatMapping(texture->desc.format).srvFormat;
    ...
    void RendererInterfaceD3D11::applyState(const DrawCallState& state, uint32_t denyStageMask)
    {
        ...
        if ((denyStageMask & StageMask::DENY_INPUT_STATE) == 0)
        {
            ...
            {
                context->IASetIndexBuffer(state.indexBuffer->resource.Get(), GetFormatMapping(state.indexBufferFormat).srvFormat, state.indexBufferOffset);
            }
            ...
            for (uint32_t i = 0; i < state.vertexBufferCount; i++)
            {
                ...
                ID3D11Buffer* pBuffer = binding.buffer->resource.Get();
    ...
    ID3D11ShaderResourceView* RendererInterfaceD3D11::getSRVForBuffer(BufferHandle buffer, Format::Enum format)
    {
    ...
    ID3D11UnorderedAccessView* RendererInterfaceD3D11::getUAVForBuffer(BufferHandle buffer)
    {
