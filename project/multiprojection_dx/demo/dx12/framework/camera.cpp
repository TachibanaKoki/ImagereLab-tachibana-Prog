...
namespace Framework
{
	...
	void PerspectiveCamera::SetReverseProjection(
		float vFOV,
		float aspect,
		float zNear)
	{
		m_projection = perspProjD3DStyleReverse(vFOV, aspect, zNear);
		UpdateWorldToClip();
	}
	...
	void FPSCamera::Update(float timestep)
	{
		...
		if (m_keysDown[CAM_MOVE_FORWARD])
			m_pos += m_viewToWorld.m_linear[2] * moveStep;
		if (m_keysDown[CAM_MOVE_BACKWARD])
			m_pos -= m_viewToWorld.m_linear[2] * moveStep;
		if (m_keysDown[CAM_STRAFE_LEFT])
			m_pos -= m_viewToWorld.m_linear[0] * moveStep;
		if (m_keysDown[CAM_STRAFE_RIGHT])
			m_pos += m_viewToWorld.m_linear[0] * moveStep;
		if (m_keysDown[CAM_MOVE_UP])
			m_pos += m_viewToWorld.m_linear[1] * moveStep;
		if (m_keysDown[CAM_MOVE_DOWN])
			m_pos -= m_viewToWorld.m_linear[1] * moveStep;
	...
	bool FPSCamera::HandleWindowsMessage(UINT message, WPARAM wParam, LPARAM lParam)
	{
		switch (message)
		{
			case WM_KEYDOWN:
			...
			case WM_KEYUP:
			{
				...
				{
					m_keysDown[mappedKey] = false;
					return true;
				}
			}
		}

		return super::HandleWindowsMessage(message, wParam, lParam);
	}
