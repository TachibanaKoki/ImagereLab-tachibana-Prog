...
namespace util
{
	...
	float4x4 perspProjD3DStyleReverse(float left, float right, float bottom, float top, float zNear)
	{
		...
		float xScale = invZNear / (right - left);
		float yScale = invZNear / (top - bottom);

		return makefloat4x4(
					2.0f * xScale, 0, 0, 0,
					0, 2.0f * yScale, 0, 0,
					-(left + right) * xScale, -(bottom + top) * yScale, 0, invZNear,
					0, 0, 1, 0);
	}
	...
	{
		float invZNear = 1.0f / zNear;
		float yScale = invZNear / tanf(0.5f * verticalFOV);
		float xScale = yScale / aspect;
